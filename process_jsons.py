...
'''
input_path = "../annotations_train/"
output_path = "../annotations_train/gaps/"

for dirpath, dnames, fnames in os.walk(input_path):
    for f in fnames:
        if f.endswith(".txt") and not f.startswith('._'):
            print(os.path.join(dirpath, f))
            segments = find_gaps(os.path.join(dirpath, f),tags)
            print(segments)
            with open(output_path + os.path.splitext(f)[0] + '_gaps.txt', 'w', newline='') as csvfile:
                writer = csv.writer(csvfile, delimiter=',')
                for seg in segments:
                    writer.writerow([ seg[0], seg[1],'gap'])
'''
def jsons2csv( output_path = "./MAVD/audio_segments/",mavd_path = "MAVD/", split="train", mode = "coarse", split_out = "train"):
    ...
    # Download MAVD if it isn't found on ../
    MAVD_files = []
    if not os.path.exists("./MAVD/audio_train/"):
        # name of the audio files in the dataset
        MAVD_files.append('audio_train.zip')
    ...
    '''
    audio_path = os.path.join(mavd_path,"audio", split)
    #i=0
